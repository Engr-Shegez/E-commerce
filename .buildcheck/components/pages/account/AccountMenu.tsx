...
import { accountMenuItems } from "./accountMenuData";

const AccountMenu = () => {
  const pathname = usePathname();
  const router = useRouter();
  if (pathname === "/account/account") return null;
  const currentPage = accountMenuItems.find((item) => item.href === pathname);
  ...
  return (
    <div className="border-b pb-2">
      ...
      <div className="lg:hidden">
        <Select
          defaultValue={
            pathname && accountMenuItems.some((item) => item.href === pathname)
              ? pathname
              : ""
          }
